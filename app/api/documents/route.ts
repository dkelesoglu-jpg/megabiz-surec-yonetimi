...
export async function GET(request: Request) {
  try {
    ...
    const db = getDb();
    if (download) {
      const { data: row } = await db.from("documents").select("*").eq("id", download).eq("company_id", companyId).maybeSingle();
    ...
    const { data } = await db.from("documents").select("*").eq("company_id", companyId).order("created_at", { ascending: false });
    return Response.json({ documents: camelizeKeys(data ?? []) });
  ...
  try {
    ...
    const { data: row, error } = await getDb()
      ...
      .insert({
        ...
        size: file.size,
        category: String(form.get("category") || "Diğer"),
...
export async function DELETE(request: Request) {
  try {
    ...
    const db = getDb();
    const { data: row } = await db.from("documents").select("*").eq("id", id).eq("company_id", companyId).maybeSingle();
    if (!row) return Response.json({ error: "Dosya bulunamadı" }, { status: 404 });
